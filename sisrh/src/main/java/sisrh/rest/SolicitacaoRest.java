...
public class SolicitacaoRest {
	...
	@Path("{id}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response obterSolicitacao(@PathParam("id") Integer _id) throws Exception {
		try {
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response incluiSolicitacao(Solicitacao solicitacao) {
		try {
			Solicitacao emp = Banco.incluirSolicitacao(solicitacao);
			return Response.ok().entity(emp).build();
		}catch (Exception e) {
			return Response.status(Status.INTERNAL_SERVER_ERROR).entity("{ \"mensagem\" : \"Falha na inclusao da Solicitacao!\" , \"detalhe\" :  \""+ e.getMessage() +"\"  }").build();
		}		
	}
